...
import { AnimeActions, fromAnime } from '@findAnime/shared/anime';
import { fromCategory } from '@findAnime/shared/category';
import { Category } from '@findAnime/shared/category/models/index';
...
import { AnimeListComponentState } from '../model';
        ...
        <ng-container *ngIf="(status$ | async) as status">
          <ng-container *ngIf="status !== 'pending' || componentStatus?.page > 0; else loader">
            <ng-container *ngIf="status !== 'error'; else serverError">
              <ng-container *ngIf="info?.animeList?.length > 0; else noData">

                <app-item-list
                  *ngFor="let anime of info?.animeList; trackBy: trackById"
                  [animeManga]="anime"
                  (openSingleCardModal)="openSingleCardModal($event)">
                </app-item-list>

                <app-infinite-scroll
                  [slice]="info?.animeList?.length"
                  [status]="status"
                  [total]="info?.total"
...
export class AnimeListPage {
  ...
  status$ = this.store.select(fromAnime.selectStatus).pipe(shareReplay(1));
  trigger = new EventEmitter<AnimeListComponentState>();
  componentStatus: AnimeListComponentState = {
    page: 0,
    filter: {},
    reload: false
  };
  ...
  info$ = this.trigger.pipe(
    ...
    concatLatestFrom(() => this.store.select(fromAnime.selectFilters)),
    tap(([{page, reload, filter}, storeFilters]) => {
      ...
      if(!!reload || page > 0){
        this.store.dispatch(AnimeActions.loadAnimeList({page, filter}))
      }
    }),
    switchMap(() =>
      this.store.select(fromAnime.selectAnimeList).pipe(
        concatLatestFrom(() => [
          this.store.select(fromAnime.selectPage),
          this.store.select(fromAnime.selectTotalCount)
        ]),
        map(([animeList = [], page = 0, total = 0]) => ({animeList, page, total}))
      )
    )
    // ,tap(d => console.log(d))
  );
